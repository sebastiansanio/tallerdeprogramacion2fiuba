...
function print_whole_category_list_solicitar($category=NULL, $displaylist=NULL, $parentslist=NULL, $depth=-1, $showcourses = true) {
    global $CFG;
    ...
    if ($category) {
        if ($category->visible or has_capability('moodle/category:viewhiddencategories', get_context_instance(CONTEXT_SYSTEM))) {
            print_category_info_solicitar($category, $depth, $showcourses);
        } else {
            return;  // Don't bother printing children of invisible categories
        }
    ...
    if ($categories = get_child_categories($category->id)) {   // Print all the children recursively
        ...
        foreach ($categories as $cat) {
            ...
            $down = $last ? false : true;
            $first = false;
            print_whole_category_list_solicitar($cat, $displaylist, $parentslist, $depth + 1, $showcourses);
        }
    }
}

function print_whole_category_list_aceptar_solicitud($category=NULL, $displaylist=NULL, $parentslist=NULL, $depth=-1, $showcourses = true) {
    global $CFG;
    // maxcategorydepth == 0 meant no limit
    if (!empty($CFG->maxcategorydepth) && $depth >= $CFG->maxcategorydepth) {
        return;
    ...
    if ($category) {
        if ($category->visible or has_capability('moodle/category:viewhiddencategories', get_context_instance(CONTEXT_SYSTEM))) {
            print_category_info_aceptar_solicitud($category, $depth, $showcourses);
        } else {
            return;  // Don't bother printing children of invisible categories
        }
    ...
    if ($categories = get_child_categories($category->id)) {   // Print all the children recursively
        ...
        foreach ($categories as $cat) {
            ...
            print_whole_category_list_aceptar_solicitud($cat, $displaylist, $parentslist, $depth + 1, $showcourses);
        }
    }
}
...
function print_category_info_aceptar_solicitud($category, $depth=0, $showcourses = false) {
    ...
    if ($showcourses and $coursecount) {
        ...
        if ($courses && ($limit || $CFG->maxcategorydepth == 0)) {
            foreach ($courses as $course) {
				...
				if ($coursedata = $DB->get_records('enrol')) {
				...
				$link = '/enrol/manual/manage_aceptar_solicitud.php';
...
function print_category_info_solicitar($category, $depth=0, $showcourses = false) {
    global $CFG, $DB, $OUTPUT;
    $strsummary = get_string('summary');
    ...
    if ($showcourses and $coursecount) {
        ...
        if ($courses && ($limit || $CFG->maxcategorydepth == 0)) {
            foreach ($courses as $course) {
				...
				$courselink = format_string($course->fullname);
                //$courselink = html_writer::link(new moodle_url('/course/view.php', array('id'=>$course->id)), format_string($course->fullname), $linkcss);
				$solicitar = html_writer::link(new moodle_url('/course/solicitudrealizada.php', array('id'=>$course->id)), "Solicitar inscripcion", $linkcss);
                // print enrol info
                $courseicon = '';
                if ($icons = enrol_get_course_info_icons($course)) {
                    foreach ($icons as $pix_icon) {
                ...
                $coursecontent = html_writer::tag('div', $courseicon.$courselink." ".$solicitar, array('class'=>'name'));
...
function print_category_enrolment_popper($category, $depth=0, $showcourses = false) {
    global $CFG, $DB, $OUTPUT;
