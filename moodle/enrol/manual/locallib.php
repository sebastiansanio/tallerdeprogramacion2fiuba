...
class enrol_manual_potential_participant_mediador extends user_selector_base {
    protected $enrolid;
                ...
                WHERE $wherecondition AND
					  ...
					  u.id IN (
						  SELECT rol.userid
							...
							WHERE roleid = 3 AND contextid = 1 )";
        $order = ' ORDER BY u.lastname ASC, u.firstname ASC';
...
class enrol_manual_potential_participant_aprendiz_solicito extends user_selector_base {
    protected $enrolid;
        ...
        $sql = " FROM {user} u
                WHERE $wherecondition AND
					  ...
					  u.id IN (
						  SELECT rol.userid
							FROM {role_assignments} rol
							WHERE roleid = 5 AND contextid = 1 ) AND
					  u.id IN (
						  SELECT sol.userid
						  FROM {solicitud} sol
						  JOIN {enrol} e ON (e.courseid = sol.courseid AND e.id=".$this->enrolid.")
					  )
		";
        $order = ' ORDER BY u.lastname ASC, u.firstname ASC';
